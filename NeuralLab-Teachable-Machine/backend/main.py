...
def rename_class(payload: RenameClassRequest):
    ...
    # If the old directory doesn't exist, it means the class has 0 samples.
    # We don't need to rename any folder on disk, just return success.
    if not os.path.exists(old_path):
        return {"status": "success", "message": f"Class directory '{payload.old_name}' did not exist (0 samples), rename on disk skipped."}
...
def delete_class(payload: DeleteClassRequest):
    ...
    # If the directory doesn't exist, it means the class has 0 samples.
    # We can just return success as there is nothing to delete.
    if not os.path.exists(class_path):
        return {"status": "success", "message": f"Class '{payload.class_name}' did not exist (0 samples), deletion skipped."}
